...
class NamespaceService:
    ...
    def validate_namespace_name(self, name: str) -> None:
        ...
        if not re.match(r"^[a-z0-9]+(-[a-z0-9]+)*$", name):
            raise ValueError(
                f"Namespace name '{name}' must be kebab-case (lowercase letters, numbers, hyphens)"
            )
    ...
    async def create_namespace(self, name: str, description: str) -> dict[str, Any]:
        ...
        if name == GLOBAL_NAMESPACE:
            raise ValueError(
                f"Cannot create namespace '{GLOBAL_NAMESPACE}' - reserved name"
            )
    ...
    async def update_namespace(
        ...
        if new_name:
            ...
            if new_name == GLOBAL_NAMESPACE:
                raise ValueError(
                    f"Cannot rename to '{GLOBAL_NAMESPACE}' - reserved name"
                )
