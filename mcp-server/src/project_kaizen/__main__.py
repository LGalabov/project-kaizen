"""Entry point for running Project Kaizen MCP server as a module."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--transport", default=None, help="Transport type (stdio, http)"
    )
    parser.add_argument(
    ...
    asyncio.run(
        server.main(
            ...
            or os.getenv(
                "POSTGRES_URL",
                "postgresql://kz_user:kz_password@localhost:5453/kz_knowledge",
            ),
            ...
            cast(
                Literal["stdio", "http"],
                args.transport or os.getenv("KAIZEN_TRANSPORT", "stdio"),
            ),
